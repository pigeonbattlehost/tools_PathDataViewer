...
class PathDataViewer(QMainWindow):
    def __init__(self):
        ...
        preview_container.setStyleSheet("background-color: #2d2d2d; border-left: 1px solid #3e3e3e;")
        ...
        try:
            ...
            for block in path_blocks:
                ...
                if fill_match:
                    ...
                    # Фикс Android Hex (#AARRGGBB -> #RRGGBB)
                    if len(fill) == 9:
                        fill = "#" + fill[3:]
                else:
                    fill = "none"
                ...
                svg_paths.append(f'<path d="{d}" fill="{fill}" fill-opacity="{alpha}" />')
